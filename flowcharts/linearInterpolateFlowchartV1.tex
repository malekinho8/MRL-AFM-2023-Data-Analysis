...
\node (init2) [process, right=0.5cm of initCheckEnd] {Initialize: \code{elapsedTicks}};
...
\node (readValues) [process, below=0.5cm of innerCheck] {Read: \code{x\_command}, \code{x\_slope}, \code{y\_command}, \code{y\_slope}};
\node (writeValues) [process, below=0.5cm of readValues] {Write: \code{x command}\& \code{y command}using \code{linearInterpolate}};
